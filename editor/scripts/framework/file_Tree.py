...
class treeView:
    ...
    def setup_treeview(
        ...
        self.folder_image = PhotoImage(
            file=def_folder_image
        )  # Default folder path for the editor
        self.file_image = PhotoImage(
            file=def_file_image
        )  # Default file path for the editor
        ...
        treestyle.configure(
            "Treeview.Heading", background=bg_color_tree, foreground="white"
        )
        treestyle.configure("Treeview", treeareaforeground="gray", show="tree lines")
        treestyle.map(
            "Treeview",
            background=[("selected", selected_color_tree)],
    ...
    def insert_item(self, name: str, path: Path, parent: str = "") -> str:
        ...
        iid = self.treeview.insert(
            parent, tk.END, text=name, tags=("fstag",), image=self.get_icon(path)
        )
