...
class Nyxtext(CTk):
    def __init__(self):
        ...
        self.terminal = Terminal(
            self.terminal_frame
        )  # Create the terminal for the editor
        ...
        bg_color_tree = self._apply_appearance_mode(
            ThemeManager.theme["CTkFrame"]["fg_color"]
        )  # Treeview (theme colors are selected)
        selected_color_tree = self._apply_appearance_mode(
        ...
        self.settings_button = CTkButton(
            self.top_frame, text="⚙️", command=self.open_settings_window
        )
        self.settings_button.pack(side="right", padx=5)
        self.settings_button.configure(width=10)

        self.terminal_button = CTkButton(
            self.top_frame,
            text="⋕",
            command=lambda: self.toggle_frame(self.terminal_frame),
        )
        self.terminal_button.configure(width=5)
        self.terminal_button.pack(side="right", padx=5)

        self.filetree_button = CTkButton(
            self.top_frame, text="◧", command=lambda: self.toggle_frame(self.left_frame)
        )
        self.filetree_button.configure(width=5)
        self.filetree_button.pack(side="right", padx=5)

        self.bottomframe_button = CTkButton(
            self.top_frame,
            text="⬓",
            command=lambda: self.toggle_frame(self.bottom_frame),
        )
        self.bottomframe_button.configure(width=5)
        self.bottomframe_button.pack(side="right", padx=5)

        appearance_optionmenu = CTkOptionMenu(self.top_frame, values=["◑"], width=2)
        appearance_optionmenu.pack(side="right", padx=2, pady=2)
        CTkScrollableDropdown(
            appearance_optionmenu,
            values=["Light", "Dark"],
        ...
        self.remove_current_tab = CTkButton(
            self.top_frame, text="✕", command=self.tabview.remove_current_tab
        )
        self.remove_current_tab.configure(width=5)
        self.remove_current_tab.pack(side="right", padx=5)

        self.add_new_tab = CTkButton(
            self.top_frame, text="⌂", command=self.tabview.add_new_workspace
        )
        self.add_new_tab.configure(width=5)
        self.add_new_tab.pack(side="right", padx=5)

        self.add_new_codespace = CTkButton(
            self.top_frame, text="⋊", command=self.tabview.add_new_codespace
        )
        ...
        self.openFile_button = CTkButton(
            self.top_frame,
            text="📂",
            command=lambda: self.tabview.add_new_workspace_with_file(
                file_path_prompt()
            ),
        )
        ...
        self.remove_top_frame = CTkButton(
            self.bottom_frame,
            text="⬒",
            command=lambda: self.toggle_frame(self.top_frame),
        )
        ...
        directory_label.configure(
            width=100,
            font=("JetBrainsMono Nerd Font", 12, "bold"),
            fg_color=f"{ThemeManager.theme['CTkLabel']['fg_color']}",
        )
        ...
        set_opacity(
            self.terminal_frame, value=0.8
        )  # Set the opacity for the terminal frame for the editor
